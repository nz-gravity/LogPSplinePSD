...
# One strong color for the inferred PSD.
POSTERIOR_COLOR = ManimColor("#F28E2B")
POSTERIOR_OPACITY = 0.48

# Observed data stays neutral.
PERIODOGRAM_COLOR = ManimColor("#969696")
...
def basis_colors(
    ...
    """
    Bright ordered basis colors.

    Turbo is intentionally used here because the colors are explanatory:
    each basis function and corresponding weight bar share a color.
    """
...
def nearest_basis_matching(
    ...
    """
    Match basis curves according to nearest center frequency.

    Returns
    -------
    matches
        List of (old_index, new_index).

    new_only
        New basis functions with no matched old curve.
...
def flattened_curve(
    ...
    """
    Copy a basis curve and collapse it onto the y=0 baseline.

    Used so genuinely new basis functions appear to grow upward.
    """
...
def make_posterior_band(
    ...
    return Polygon(
        *(upper + lower),
        ...
        fill_opacity=POSTERIOR_OPACITY,
        stroke_width=0,
    )
...
def make_periodogram(
    ...
    """
    Draw the observed periodogram as a faint grey connected line.

    It is deliberately static throughout the animation because every
...
class LogPSplineHero(
    ...
    def construct(
        ...
        def transition_to(
            ...
            # Keep explicit references to all temporary objects so
            # nothing survives accidentally on downward transitions.
            temporary_objects = []
            ...
            # Existing curves morph into nearest new curves.
            ...
            # HARD CLEANUP
            #
            # Explicitly remove all children of the old groups,
            # the group objects themselves, and any temporary
            # growing objects.
            #
