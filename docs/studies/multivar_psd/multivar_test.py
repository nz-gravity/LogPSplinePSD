...
class DiscreteFFT:
    ...
    def compute_fft(cls, x: np.ndarray, fs: float = 1.0) -> 'DiscreteFFT':
        ...
        y_re = np.real(x_fft)
        y_im = np.imag(x_fft)
        Z_re, Z_im = compute_cholesky_design(x_fft)
        return cls(
            y_re=jnp.array(y_re),
            y_im=jnp.array(y_im),
            Z_re=jnp.array(Z_re),
...
def setup_psplines(
    ...
    """
    Setup P-spline basis and penalty matrices for each component.
    """
...
def multivariate_psplines_model(
    ...
    for j in range(n_dim):
        ...
        phi = numpyro.sample(f"phi_delta_{j}", dist.Gamma(alpha_phi, beta_phi))
...
def reconstruct_psd_from_cholesky(log_delta_sq_samples, theta_re_samples, theta_im_samples):
    ...
    .. math::
        S(f) = T^{-1} D T^{-H}
    where D = diag(exp(log_delta_sq)), T is lower-triangular with -theta in lower triangle.
    """
...
if __name__ == "__main__":
    ...
    data = DiscreteFFT.compute_fft(x, fs=1.0)
    print(f"FFT shapes: y_re={data.y_re.shape}, Z_re={data.Z_re.shape}")
    all_bases, all_penalties = setup_psplines(
