...
def extract_metrics_from_inferencedata(idata: az.InferenceData) -> Tuple[float, float, float, float]:
    """
    ...
    if np.isnan(ci_coverage):
        print("Warning: ci_coverage not found in attributes, computing on-the-fly")
        ...
        try:
            ...
            if "psd_matrix" in idata.posterior:
                ...
                posterior_psds_real = np.zeros((n_samples, n_freq, *posterior_psds.shape[2:]), dtype=float)
                for i in range(n_samples):
                    for j in range(n_freq):
                        posterior_psds_real[i, j] = _complex_to_real(posterior_psds[i, j])
                posterior_lower = np.percentile(posterior_psds_real, 2.5, axis=0)
                posterior_upper = np.percentile(posterior_psds_real, 97.5, axis=0)
                true_psd_real = np.zeros_like(true_psd, dtype=float)
                for j in range(n_freq):
                    true_psd_real[j] = _complex_to_real(true_psd[j])
                ci_coverage = np.mean((true_psd_real >= posterior_lower) & (true_psd_real <= posterior_upper))
            # Compute coverage for univariate case
            elif "psd" in idata.posterior_psd:
                psd_samples = idata.posterior_psd["psd"].values  # (pp_draw, freq)
                posterior_lower = np.percentile(psd_samples, 2.5, axis=0)
                posterior_upper = np.percentile(psd_samples, 97.5, axis=0)
                ci_coverage = np.mean((true_psd >= posterior_lower) & (true_psd <= posterior_upper))
            else:
                print("Warning: PSD data not found in posterior for coverage calculation")
                ci_coverage = np.nan
...
def extract_study_parameters(idata: az.InferenceData) -> Tuple[Optional[int], Optional[int]]:
    """
...
def read_or_create_summary(results_dir: str = RESULTS_DIR,
                           csv_file: str = CSV_FILE) -> pd.DataFrame:
    """
    ...
    if os.path.exists(csv_file) and not CLEAN:
        ...
        for fname in tqdm(nc_files):
            try:
                ...
                # Extract metrics
                runtime, ess, riae, coverage = extract_metrics_from_inferencedata(idata)
...
def plot_with_errorbars(df: pd.DataFrame, x_col: str, y_col: str, group_col: str,
    ...
    plt.errorbar(x_values, y_means, yerr=y_stds, fmt='o-', capsize=4, markersize=6)
...
def create_all_plots(df: pd.DataFrame, output_dir: str = "plots") -> None:
    ...
    plot_df = df.dropna(subset=['N', 'K', 'runtime', 'ess', 'riae', 'coverage'])
    ...
    # ESS vs N (log-log)
    plot_with_errorbars(
        plot_df, x_col="N", y_col="ess", group_col="K",
        xlabel="N (Data Size)", ylabel="Effective Sample Size (ESS)",
        save_path=os.path.join(output_dir, "ess_vs_N.png")
    )

    # RIAE vs N (log-log)
    plot_with_errorbars(
        plot_df, x_col="N", y_col="riae", group_col="K",
        xlabel="N (Data Size)", ylabel="RIAE",
        save_path=os.path.join(output_dir, "riae_vs_N.png")
    )

    # Coverage vs N (linear)
    plot_with_errorbars(
        plot_df, x_col="N", y_col="coverage", group_col="K",
        xlabel="N (Data Size)", ylabel="Coverage Probability",
        save_path=os.path.join(output_dir, "coverage_vs_N.png")
    )

    # ESS vs K (log-log)
    plot_with_errorbars(
        plot_df, x_col="K", y_col="ess", group_col="N",
        xlabel="K (Number of Knots)", ylabel="Effective Sample Size (ESS)",
        save_path=os.path.join(output_dir, "ess_vs_K.png")
    )

    # RIAE vs K (log-log)
    plot_with_errorbars(
        plot_df, x_col="K", y_col="riae", group_col="N",
        xlabel="K (Number of Knots)", ylabel="RIAE",
        save_path=os.path.join(output_dir, "riae_vs_K.png")
    )

    # Coverage vs K (linear)
    plot_with_errorbars(
        plot_df, x_col="K", y_col="coverage", group_col="N",
        xlabel="K (Number of Knots)", ylabel="Coverage Probability",
        save_path=os.path.join(output_dir, "coverage_vs_K.png")
    )


def main(results_dir: str = RESULTS_DIR, csv_file: str = CSV_FILE,
         create_plots: bool = True, output_dir: str = "plots") -> pd.DataFrame:
    """
