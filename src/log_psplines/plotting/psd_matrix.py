...
def plot_psd_matrix(
    ...
    fig, axes = plt.subplots(
        n_channels, n_channels, figsize=(4 * n_channels, 4 * n_channels)
    )
    if n_channels == 1:
        axes = [[axes]]
    elif n_channels == 2:
    ...
    for i in range(n_channels):
        for j in range(n_channels):
            ...
            if i == j:  # Diagonal elements (auto-spectra)
                ...
                if (
                    "true_psd" in idata.attrs
                    and idata.attrs["true_psd"] is not None
                ):
                    true_psd_matrix = idata.attrs["true_psd"]
                    if true_psd_matrix.ndim == 3 and true_psd_matrix.shape[
                        0
                    ] == len(freq):
                        ax.plot(
                            freq,
                            np.real(true_psd_matrix[:, i, i]),
                            "r-",
                            alpha=0.7,
                            label="True PSD",
                        )
                ...
                if i > j:
                    ...
                    coh_samples = np.abs(psd_samples[:, :, i, j]) ** 2 / (
                    ...
                    if (
                        ...
                        if true_psd_matrix.ndim == 3 and true_psd_matrix.shape[
                            ...
                            true_coh = np.abs(
                                ...
                                np.abs(true_psd_matrix[:, i, i])
                                * np.abs(true_psd_matrix[:, j, j])
                            )
                            ax.plot(
                                freq,
                                true_coh,
