...
class PSDDiagnostics:
    ...
    def __init__(
        ...
        if self.psd.shape[0] != self.freqs.shape[0]:
            raise ValueError(f"psd and freqs must have same length, got {self.psd.shape[0]} and {self.freqs.shape[0]}")
        ...
        self.rayleigh_spectrum, self.rayleigh_freqs = self._calculate_rayleigh_spectrum(
            fftlength, overlap
        )

        # Calculate reference Rayleigh spectrum if reference PSD provided
        if self.reference_psd is not None:
            self.rayleigh_spectrum_ref = self._calculate_rayleigh_spectrum_from_psd(
    ...
    def _calculate_rayleigh_spectrum(
        ...
        for i in range(n_segments):
            ...
            freqs_seg, psd_seg = scipy.signal.welch(
                segment, fs=self.fs, window=window, nperseg=len(segment),
    ...
    def _calculate_rayleigh_spectrum_from_psd(
        ...
        rayleigh_ref_interp = np.interp(self.freqs, freqs_seg, rayleigh_spectrum_ref)
    ...
    def plot_diagnostics(
        ...
        welch_pass_percent = self._rayleigh_pass_percent(self.rayleigh_spectrum)
        ax2.plot(self.freqs, self.rayleigh_spectrum, color="C0", linewidth=1, label=f"Welch PSD [{welch_pass_percent}%]")
        if self.reference_psd is not None and self.rayleigh_spectrum_ref is not None:
            ref_pass_percent = self._rayleigh_pass_percent(self.rayleigh_spectrum_ref)
            ax2.plot(self.freqs, self.rayleigh_spectrum_ref, color="C1", linestyle="--", linewidth=1,
                     label=f"Reference PSD [{ref_pass_percent}%]", alpha=0.4)

        ax2.axhline(1.5, color="blue", linestyle=":", alpha=1, label="Non-Gaussian (R=1.5)", zorder=10)
        ax2.axhline(1.0, color="black", linestyle=":", alpha=1, label="Gaussian (R=1)", zorder=10)
        ax2.axhline(0.5, color="red", linestyle=":", alpha=1, label="Coherent (R=0.5)", zorder=10)

        # Highlight coherent features for estimated PSD
        coherent_mask = self.rayleigh_spectrum < 0.5
        if np.any(coherent_mask):
            ax2.fill_between(self.freqs, 0, self.rayleigh_spectrum,
                             where=coherent_mask, color="C0", alpha=0.2)
        ...
        if self.wh_f is not None and len(self.wh_f) > 0:
            ...
            fbins, pvals = self._fbins_anderson_p_value(freqs_for_whitening, self.wh_f, bin_width_Hz, fmin, fmax)

            if len(fbins) > 0:
                welch_pass = self._ad_pass_percent(pvals)
                ax3.plot(fbins, pvals, alpha=0.7, color="C0", label=f"Welch PSD [{welch_pass}%]")

        # Compute p-values for reference PSD if available
        if self.reference_psd is not None and self.wh_f_ref is not None and len(self.wh_f_ref) > 0:

            freqs_for_whitening_ref = np.linspace(fmin, fmax, len(self.wh_f_ref))
            fbins_ref, pvals_ref = self._fbins_anderson_p_value(freqs_for_whitening_ref, self.wh_f_ref, bin_width_Hz,
                                                                fmin, fmax)
            if len(fbins_ref) > 0:
                ref_pass = self._ad_pass_percent(pvals_ref)
                ax3.plot(fbins_ref, pvals_ref, alpha=0.7, color="C1", label=f"Reference PSD [{ref_pass}%]")

        ax3.axhline(1e-2, color="red", linestyle=":", alpha=0.7, label="p=0.01 threshold", zorder=10)
    ...
    def _prepare_whitening_data(self) -> None:
        ...
        fft_freqs = np.fft.fftfreq(n, 1 / self.fs)[1: n // 2]  # Positive freqs, skip DC
        ...
        if self.reference_psd is not None:
            asd_ref_interp = np.sqrt(np.interp(fft_freqs, self.freqs, self.reference_psd))
            # Avoid division by zero
            asd_ref_interp = np.where(asd_ref_interp == 0, np.finfo(float).eps, asd_ref_interp)
            self.wh_f_ref = self.h_f * np.sqrt(4.0 / self.duration) / asd_ref_interp
        else:
            self.wh_f_ref = None
        ...
        for ii in idxs:
            ...
            if len(block) > 5 and len(freq_block) > 0:
                ...
                if fmin <= center_freq <= fmax:
                    pvals.append(anderson_p_value(block, freq_block, fmin, fmax))
                    fbins.append(center_freq)

        return np.array(fbins), np.array(pvals)

    def _rayleigh_pass_percent(self, rayleigh_stat):
        return int(np.sum((rayleigh_stat >= 0.5) & (rayleigh_stat <= 1.5)) / len(rayleigh_stat) * 100)

    def _ad_pass_percent(self, p_values, threshold=0.01):
        return int(np.sum(p_values > threshold) / len(p_values) * 100 if len(p_values) > 0 else 0)
    ...
    def summary_stats(self) -> dict:
        ...
        stats = {
            ...
            'analysis_freq_range_hz': (self.freqs[0], self.freqs[-1]) if len(self.freqs) > 0 else (0, 0),
        ...
        valid_rayleigh = self.rayleigh_spectrum[~np.isnan(self.rayleigh_spectrum)]
        if len(valid_rayleigh) > 0:
            stats['rayleigh_mean'] = np.mean(valid_rayleigh)
            stats['rayleigh_median'] = np.median(valid_rayleigh)
            stats['fraction_coherent'] = np.sum(valid_rayleigh < 0.5) / len(valid_rayleigh)
            stats['fraction_gaussian'] = np.sum(np.abs(valid_rayleigh - 1.0) < 0.1) / len(valid_rayleigh)

        # Rayleigh statistics for reference PSD (if available)
        if self.rayleigh_spectrum_ref is not None:
            valid_rayleigh_ref = self.rayleigh_spectrum_ref[~np.isnan(self.rayleigh_spectrum_ref)]
            if len(valid_rayleigh_ref) > 0:
                ...
                stats['fraction_coherent_ref'] = np.sum(valid_rayleigh_ref < 0.5) / len(valid_rayleigh_ref)
                stats['fraction_gaussian_ref'] = np.sum(np.abs(valid_rayleigh_ref - 1.0) < 0.1) / len(
                    valid_rayleigh_ref)
...
def anderson_p_value(
    ...
    critical_values = np.array([
        0.200, 0.300, 0.400, 0.500, 0.576, 0.656, 0.787, 0.918, 1.092, 1.250,
        1.500, 1.750, 2.000, 2.500, 3.000, 3.500, 4.000, 4.500, 5.000, 6.000,
        7.000, 8.000, 10.000
    ])
    significance_levels = np.array([
        0.90, 0.85, 0.80, 0.75, 0.70, 0.60, 0.50, 0.40, 0.30, 0.25,
        0.20, 0.15, 0.10, 0.05, 0.01, 0.005, 0.0025, 0.001, 0.0005, 0.0002,
