...
class LogPSplines:
    ...
    **Validation Rules:**
    ...
    def __call__(self, weights: jnp.ndarray | None = None) -> jnp.ndarray:
        """
        Evaluate the log power spectral density.

        Computes the weighted sum of B-spline basis functions.
...
def build_spline(
    ...
    return jnp.einsum("ij,j->i", ln_spline_basis, weights)
