...
from log_psplines.metropolis_hastings import MetropolisHastingsSampler
...
def test_metropolis_hastings(mock_pdgrm: Periodogram, outdir):
    ...
    inference_data = sampler.sample(
        n_samples=5000, n_warmup=2000, thin=1, verbose=True
    )

    weights = get_weights(inference_data, thin=10)

    out = f"{outdir}/out_metropolis_hastings"
    os.makedirs(out, exist_ok=True)

    fig, ax = plot_pdgrm(mock_pdgrm, spline_model, weights)
    fig.savefig(os.path.join(out, f"test_metropolis_hastings.png"))
    plt.close(fig)
