...
def test_basis_log_vs_linear(mock_pdgrm: Periodogram, outdir):
    ...
    def create_bspline_basis(knots, degree, domain, n_points=200):
        ...
        full_knots = np.concatenate([
            np.repeat(domain[0], degree),
            knots,
            np.repeat(domain[1], degree)
        ])
    ...
    knots_log_norm = (np.log10(knots_log) - np.log10(freq_min)) / (np.log10(freq_max) - np.log10(freq_min))

    # Create basis functions
    x_linear_norm, basis_linear = create_bspline_basis(knots_linear_norm, degree, [0, 1], 300)
    x_log_norm, basis_log = create_bspline_basis(knots_log_norm, degree, [0, 1], 300)

    # Convert back to frequency domain
    freq_linear_basis = freq_min + (freq_max - freq_min) * x_linear_norm
    freq_log_basis = 10 ** (np.log10(freq_min) + x_log_norm * (np.log10(freq_max) - np.log10(freq_min)))
    ...
    for i in range(basis_linear.shape[1]):
        ax1.plot(freq_linear_basis, basis_linear[:, i], 'b-', alpha=0.7, linewidth=1,
                 label='Linear knots' if i == 0 else '',
                 marker='.')

    for i in range(basis_log.shape[1]):
        ax1.plot(freq_log_basis, basis_log[:, i], 'r-', alpha=0.7, linewidth=1,
                 label='Log knots' if i == 0 else '',
                 marker='.')

    # Add knots
    ax1.scatter(knots_linear, np.full(len(knots_linear), -0.05), s=80, c='blue', alpha=0.8,
                marker='|', linewidth=3, label='Linear Knots')
    ax1.scatter(knots_log, np.full(len(knots_log), -0.1), s=80, c='red', alpha=0.8,
                marker='|', linewidth=3, label='Log Knots')

    ax1.set_xlabel('Frequency (Hz)', fontsize=12)
    ax1.set_ylabel('Basis Function Value', fontsize=12)
    ax1.set_title('P-spline Basis Functions (Linear Scale)', fontweight='bold', fontsize=14)
    ax1.grid(False)
    ax1.set_ylim(-0.15, 1.1)

    # Right plot: Log scale
    for i in range(basis_linear.shape[1]):
        ax2.plot(freq_linear_basis, basis_linear[:, i], 'b-', alpha=0.7, linewidth=1,
                 label='Linear knots' if i == 0 else '',
                 marker='.')

    for i in range(basis_log.shape[1]):
        ax2.plot(freq_log_basis, basis_log[:, i], 'r-', alpha=0.7, linewidth=1,
                 label='Log knots' if i == 0 else '',
                 marker='.')

    # Add knots
    ax2.scatter(knots_linear, np.full(len(knots_linear), -0.05), s=80, c='blue', alpha=0.8,
                marker='|', linewidth=3)
    ax2.scatter(knots_log, np.full(len(knots_log), -0.1), s=80, c='red', alpha=0.8,
                marker='|', linewidth=3)

    ax2.set_xlabel('Frequency (Hz)', fontsize=12)
    ax2.set_title('P-spline Basis Functions (Log Scale)', fontweight='bold', fontsize=14)
    ax2.set_xscale('log')
    ax2.grid(False)
